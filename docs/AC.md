...
| FR-008 | 지원금 규모 추출 (데이터) | AC-028 ~ AC-030 | 구현(데이터) — AC-028~030 Pass(파서 단위 18케이스 + 실DB 상속·멱등 통합). **라이브 전수 검수 3회 반복: 오추출 5유형 발견→규칙 승격→최종 57건(기업마당 17·온통청년 40) 오추출 0**. K-Startup 0건(금액이 첨부에만 — §6-E, 그룹 상속·Phase 2가 경로). 서빙·표시는 후속 |
