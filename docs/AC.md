...
| FR-008 | 지원금 규모 추출 (데이터) | AC-028 ~ AC-030 | 구현(데이터) — AC-028~030 Pass(파서 단위 19케이스 + 실DB 상속·멱등 통합). **라이브 검수 반복(오추출 5유형→규칙 승격) 후 141건 채움**(K-Startup 80·온통청년 41·기업마당 20 — 후처리 재산출이 기존 행 본문도 커버). 최신 공고 전수 + K-Startup 상위 표본 검수 오추출 0. 서빙·표시는 후속 |
