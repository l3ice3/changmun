...
When:  원문에서 마감일(또는 제목·모집시작일·지원 대상 문구·금액 표기)이 변경된 fixture로 재수집한다
...
And:   **eligibility_detail(지원 대상·자격 문구)만 바뀌어도 강등되고,
       그 실체의 manual_*·manual_tagged_at이 비워진다** —
       민간은 구조화 필드가 없어 검수자가 이 문구를 읽고 페르소나를 매기므로(§6-F 규칙 9),
       문구가 바뀌면 태그의 근거가 사라진다. 이 필드를 비교에서 빼면
       낡은 자격 설명과 낡은 페르소나가 **둘 다** 무검증으로 계속 노출된다
```
**검증**
- [ ] 자동: pytest — 핵심 필드 변경 → pending 강등 / 비핵심(요약·기관 표기) 변경 → approved 유지 (실DB 재수집 통합)
- [ ] 자동: 실DB 통합 — `eligibility_detail`만 변경 → 강등 + `manual_*`·`manual_tagged_at` 초기화. 비교 튜플에서 이 필드를 빼면 실패해야 한다
