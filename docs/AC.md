...
- [ ] **수동(웹, Negative)**: revalidation 엔드포인트를 일부러 실패시킨 상태(5xx·타임아웃)로 강등 → ① 판정은 DB에 남고 ② 리포트에 ERROR가 뜨며 ③ 재시도 명령(`--flush-cache`) 실행 후 세 경로에서 사라지는지. 실패가 조용히 넘어가면 무효화가 없는 것과 같다(PRD FR-010 9항)
