...
| FR ID | 기능명 | 연결 AC | 상태 |
|---|---|---|---|
| FR-001 | 공고 수집 파이프라인 | AC-001 ~ AC-005 | 부분 구현 — K-Startup(2026-06-11, 29,046건)·**온통청년(2026-06-12, 322건·스킵0·미지값0) 라이브 검증 완료**, AC-002~005 Pass / bizinfo 키 발급 대기 |
| FR-002 | dedup & 페르소나 부여 | AC-006 ~ AC-010 | 구현 — AC-006~010 자동 테스트 Pass(실DB 통합 포함). 라이브(2026-06-12): 29,383건 → 19그룹, **전수 검수 오합치 0**(경계 1건 기록). 임계 0.85 + 실질 토큰 차이 거부 튜닝(PRD §11-8 절차 수행) |
| FR-003 | 페르소나 필터 리스트 | AC-011 ~ AC-014 | 구현 — 도메인 산식 단위 + 리포지토리 슬라이스(Testcontainers) + 리스트 API E2E(MockMvc) 자동 테스트 Pass. AC-011·012·013·014·CC-05 충족 |
| FR-004 | 공고 상세 + 용어풀이 | AC-015 ~ AC-018 | 미구현 |
| FR-005 | 키워드 검색 | AC-019 ~ AC-021 | 미구현 |
| FR-006 | 찜(익명·로컬) | AC-022 ~ AC-024 | 미구현 |
...
- [ ] PRD Out-of-Scope 기능이 구현돼 있지 않음 (로그인·추천·알림·관리자 UI 없음) ← 과잉 구현 방지
