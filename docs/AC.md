...
- [ ] 자동: 동 — **수동 태그 잔존 검증**: 공공 + 승인 민간이 한 그룹이고 민간 검수 때 `manual_*`를 매긴 상태에서 그 민간 멤버를 **재검수 지정**하면, 공공 멤버로 계속 노출되는 공고의 `targetStartupStage`·`targetAudienceType`에 그 태그가 **남지 않는지**(§2-C 불변식 6). 이어서 **그 실체가 태깅 큐에도 잡히지 않는지** — 큐 조건이 `is_visible`이면 공공 때문에 계속 잡혀, 작업자가 큐를 정상 처리하는 것만으로 아직 `pending`인 멤버를 근거로 태그가 되살아난다(§6-F 규칙 10)
- [ ] 자동: 동 — **승인 민간 2건 중 1건만 재검수 지정**해도 태그가 비워지는지. 남은 승인 멤버가 그 태그의 근거라는 보장이 없다(provenance 미기록). 비운 뒤에는 태깅 큐가 다시 잡아 **남은 멤버를 근거로 새로 매기게** 되는 것까지 확인한다
- [ ] 자동: 병합 배치 실DB 통합 — **마지막 승인 멤버를 재검수 지정**해 그룹 멤버가 전부 `pending`·`rejected`가 된 경우에도 재병합이 실패하지 않고 `representative_record_id`가 채워진 채 `is_visible=false`가 되는지(§6-D 단계 5의 fallback — `NOT NULL`이라 비울 수 없다). `manual_*`는 노출 자격과 무관하게 합쳐지므로 **상속 donor 제한으로는 안 걸리는 별개 경로**다 — 위 항목의 `source_record.target_*` 표식으로는 잡히지 않는다
...
- [ ] 자동: 실DB 경합 통합 — **두 태깅 작업자가 같은 항목·같은 `opportunity.updated_at`을 읽고 제출**하면 먼저 커밋한 쪽만 성공하는지. 성공한 태깅이 `updated_at`을 올리지 않으면 두 번째도 통과해 **앞사람의 태그를 조용히 덮는다**(§6-F 규칙 5의 CAS — 거는 쪽만 있고 올리는 쪽이 없으면 가드가 무의미하다)
- [ ] 자동: 실DB 경합 통합 — **태깅 작업자가 큐를 읽은 뒤 마지막 승인 멤버가 재검수 지정되면** 늦게 도착한 태깅 제출이 거부되는지. 큐 술어는 조회 시점에만 걸리므로, 이게 없으면 불변식 6이 방금 비운 태그를 작업자가 **정상 절차만 밟아도** 되살린다(§6-F 규칙 5)
...
- [ ] **실제 v1 덤프로 업그레이드 1회 리허설** — 이름 충돌(인덱스·제약·시퀀스)뿐 아니라 **`NOT NULL` 컬럼 추가 순서**(nullable ADD → 백필 → `SET NOT NULL`)와 북마크 리매핑 순서까지 한 번에 검증된다. — RENAME이 인덱스·제약 이름을 옮기지 않아 `opportunity_pkey` 포함 7개가 신 테이블과 충돌한다(§2-D 3단계). 이름 정리가 신 테이블 생성보다 앞서는지를 리뷰가 아니라 **실행으로** 고정한다
...
- [ ] 웹 캐시 무효화 경로 동작 확인 — **판정과 태깅 완료 양쪽**(태깅은 `target_*`를 바꾸므로 빠뜨리면 최대 24h 옛 태그 노출) · 홈·`/search`·상세 3곳 + 실패 복구 3경로 + **미인증·시크릿 불일치 요청이 무효화를 수행하지 않고 401/403** — PRD FR-011 9항). 태그 전체를 날리는 공개 경로라 무인증이면 외부에서 ISR을 상시 비워 origin에 부하를 밀어 넣을 수 있다
