...
- **같은 공고인지 판정**은 제목·기관·기간 유사도(0.85 이상)로. *지역*은 **집합이 정확히 같을 때만** 병합한다 — 부분 겹침(`{서울,경기}` vs `{서울}`)이나 다른 지역(`{서울}` vs `{부산}`)은 **합치지 않는다**(대표 1건만 노출되면 일부 지역 필터에서 누락되므로). 한쪽 지역이 NULL이면 비교를 생략한다.
