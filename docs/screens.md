...
- **목적:** 프로필 이미지 관리 + 계정 확인. 톱바의 이메일 텍스트 대신 **아바타**(업로드 이미지 또는 디폴트 창문 모티프)가 뜨고, 클릭하면 이 화면.
- **구성:** 아바타(96px) + 사진 변경(JPEG/PNG/WebP·1MB 이하 — 클라·서버 이중 검증) + 기본 이미지로 되돌리기 · 계정 정보(이메일·로그인 방식) · 로그아웃. 비로그인 접근 시 로그인 유도.
- **도출 API:** `GET/PUT/DELETE /api/v1/users/me/profile-image` (api-spec §5) · `GET /api/v1/auth/me`
