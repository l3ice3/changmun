...
CREATE TABLE source_record (
    ...
    opportunity_id          BIGINT,       -- FK는 아래 ALTER로 (순환 참조 — opportunity가 아직 없다)
...
> **그래서 위 두 `CREATE TABLE`의 해당 컬럼은 일반 `BIGINT`이고, FK는 §2-C 끝의 `ALTER TABLE`
> 두 줄로 붙는다** — 인라인 `REFERENCES`로 쓰면 먼저 만드는 테이블이 아직 없는 테이블을
> 참조해 첫 문장부터 실패한다. 순환 자체는 문제가 아니다 — 한 쪽은 "이 레코드가 속한 실체",
> 다른 쪽은 "이 실체를 대표하는 레코드"로 뜻이 다르고, 둘 다 병합 배치가 같은 트랜잭션에서 쓴다.
...
CREATE TABLE opportunity (
    ...
    representative_record_id BIGINT       NOT NULL,   -- FK는 아래 ALTER로 (순환 참조)
...
-- 순환 참조 2개는 두 테이블을 만든 뒤 붙인다 (위 두 블록의 실행 순서와 이 블록이 한 세트다).
ALTER TABLE source_record ADD CONSTRAINT fk_source_record_opportunity
    FOREIGN KEY (opportunity_id) REFERENCES opportunity (id) ON DELETE SET NULL;
ALTER TABLE opportunity   ADD CONSTRAINT fk_opportunity_representative
    FOREIGN KEY (representative_record_id) REFERENCES source_record (id);
```
...
4. `target_startup_stage`·`target_audience_type` = **`is_publishable` 멤버들의 자체 파싱값 합집합 ∪ `manual_*`**.
   ...
   **`manual_*` 세 컬럼을 쓰는 주체는 검수 CLI, 그리고 태그의 근거가 사라졌을 때 값을 비우는
   ...
   초기화만 쓴다"**가 규칙이다.
5. **멤버의 노출 자격이나 병합 입력을 바꾸는 동작은 같은 트랜잭션에서 그 그룹을 재병합한다.**
   `opportunity`는 물리 테이블이라, `source_record.is_publishable`이 뒤집히거나 `manual_*`를
   손대도 **재병합 전까지 서빙은 그대로다** — 상태만 바뀌고 공고는 계속 노출되거나 계속 숨는다.
   대상은 검수 승인 · 재검수 지정 · 자격 문구 변경에 따른 태그 초기화다(§6-F 규칙 4·6).
   반려(`pending` → `rejected`)는 노출 자격이 false → false라 그대로여서 재병합이 필요 없다.
   **동작마다 규칙을 따로 달지 않는다** — 새 동작이 생기면 이 불변식이 자동으로 적용된다.
6. **수동 태그의 근거를 더는 보장할 수 없으면 `manual_*`를 비우고 태깅 큐로 보낸다.**
   태그는 실체에 붙지만 판단의 근거는 특정 멤버의 본문이다(§6-F 규칙 9). 그 연결이 끊기는 경우:
   - **그 실체의 `approved` 민간 멤버가 하나라도 줄었다**(재검수 지정·반려). 검수된 근거가
     흔들렸는데 태그만 남으면, 공공 멤버로 계속 노출되는 공고의 페르소나 필터·응답에
     **미검수 판단이 그대로 실린다** — 불변식 4가 `manual_*`를 노출 자격과 무관하게 합치기
     때문이고, 단계 6의 donor 제한(`is_publishable`)으로는 안 걸린다.
     **"승인 멤버가 남아 있으면 유지"라는 예외를 두지 않는다** — 어느 멤버를 보고 매겼는지
     기록하지 않으므로(아래), 남은 승인 멤버가 그 태그의 근거라는 보장이 없다. A·B가 모두
     승인된 실체에서 A의 자격 문구만 보고 매긴 태그는 A를 재검수 지정하는 순간 근거를 잃는데,
     B가 승인 상태라는 이유로 살려 두면 미검수 판단이 그대로 노출된다. 남은 승인 멤버가 있으면
     태깅 큐가 곧바로 다시 잡아 **그 멤버를 근거로 새로 매기게** 한다(§6-F 규칙 10).
   - **자격 문구(`eligibility_detail`)가 바뀌었다** — 검수자가 읽은 그 문장이 달라졌다(§6-F 규칙 4).

   **태깅 큐(§6-F 규칙 10)는 이 불변식의 뒷면이고 술어가 같다** — 이쪽이 "근거가 없어지면
   비운다"면 저쪽은 "근거가 있는데 안 매겼으면 잡는다"다. 둘 다 `approved` 민간 멤버의 존재를
   본다. 술어가 어긋나면 **비운 것을 곧바로 다시 채우는 왕복**이 생긴다.

   비우고 끝내는 이유는 **어느 멤버를 보고 매겼는지 기록하지 않기 때문**이다. 멤버별 provenance를
   두면 기여분만 걷어낼 수 있지만 컬럼이 늘고 불변식 4가 조건부가 된다. 민간 공고량이 소스당
   연 1~5건이라 재태깅 부담이 작아 **버리고 다시 매기는 쪽**을 택했다. 공고는 계속 노출되고
   페르소나 탭에서만 "조건 미상"으로 빠진다(절대규칙 8).
   > 그룹 분할로 태그의 근거가 흐려지는 경우도 같은 부류지만, 분할 처리 자체가 §6-D 단계 4-B의
   > 열린 항목이라 거기서 함께 정한다.
...
3. **구 이름 정리 — 3단계보다 반드시 먼저.** `ALTER TABLE ... RENAME`은 **인덱스와 제약 이름을
   ...
   - RENAME: `opportunity_pkey`→`source_record_pkey` · `uq_opportunity_source`→`uq_source_record` ·
     `idx_opportunity_*` 6종 → `idx_source_record_*` · **identity 시퀀스 `opportunity_id_seq`→
     `source_record_id_seq`**. 시퀀스도 RENAME을 따라가지 않는데, 4단계의 새 `opportunity`가
     `GENERATED BY DEFAULT AS IDENTITY`로 같은 이름을 만들려다 충돌한다 — 인덱스와 같은 함정이
     한 겹 더 있는 셈이다(`ALTER SEQUENCE ... RENAME TO`).
...
4. 신 `opportunity` 생성 후 그룹별 1행 INSERT — **id는 현재
   ...
   → **기존 상세 URL 전량 보존**(이관은 그룹을 바꾸지 않으므로 대표 id가 그대로 실체 id가 된다).
5. **순환 FK 2개를 `ALTER TABLE`로 추가**(§2-C 끝) → `source_record.opportunity_id` 역참조 채우기.
   **북마크는 값을 먼저 고치고 FK를 나중에 건다** — 순서가 이렇다:

   ① `bookmark`의 기존 FK **DROP**(RENAME 때문에 지금은 `source_record`를 가리키고 있다) →
   ② **리매핑하면 충돌할 행을 먼저 DELETE** — 한 사용자가 **같은 그룹의 행을 둘 이상** 찜했으면
   그 그룹에서 한 행만 남긴다(**대표 여부와 무관**. 대표를 찜한 행이 있으면 그것을, 없으면 `id`가
   작은 행을 남긴다). 판정 기준은 "대표인가"가 아니라 **"리매핑 후 `(user_id, opportunity_id)`가
   유일한가"**다 — 대표만 기준으로 삼으면 대표가 A→B→C로 바뀌는 동안 A와 B를 찜한 사용자처럼
   **둘 다 비-canonical인 조합**이 남아 ③에서 다시 UNIQUE를 위반한다 →
...
| v1에서 필요했던 것 | v2 |
|---|---|
| 구 §6-F 규칙 4 — 승인 강등 판정용 `raw` 스냅샷을 저장·비교 (문서 ~15줄 + 전용 코드) | **삭제.** 강등이라는 개념 자체를 없앴다 — 재수집은 내용만 갱신하고 상태는 건드리지 않는다(§6-F 규칙 4) |
| 구 §6-F 규칙 2 — `CASE` CHECK 2개 + `NULL IN (...)`은 NULL이라는 함정 주석 | `review_status NOT NULL` 4값 + CHECK 한 줄 (§2-B) |
| 구 §6-F 규칙 8·4 — 승인/강등 시 canonical 원자적 재선정 2곳 | 승인 시 그룹 1행 재병합 1곳(강등이 없어짐) |
...
4-B. **실체 배정 — 열린 항목(구현 PR에서 실측 후 확정).** 4의 파티션은 매 배치 다시 계산된다: `_seed_existing_groups`가 기존 그룹을 재검증해 stale 그룹을 해제하고, 블로킹 안에서 새 쌍이 붙는다(`dedup/engine.py`). v1은 `dedup_group_id` 숫자만 갈아 끼우면 끝이었지만 **v2는 그룹이 곧 행이라 같은 사건이 행의 생성·삭제가 된다.**
   ...
   - **그때까지의 동작**: 병합되면 흡수된 실체의 상세는 404가 되고, 그 실체를 찜한 기록도 `bookmark`의 `ON DELETE CASCADE`로 함께 사라진다(v1도 대표 교체 시 상세 URL이 흔들렸으니 더 나빠지지는 않는다). **알고 두는 손해**이지 숨은 결함이 아니며, 병합이 실제로 관측되면 그때 함께 정한다.
5. **대표 선정:** **① 노출 가능성 → ② 출처(`source_registry.canonical_rank` — K-Startup 우선, 페르소나 필드 풍부) → ③ 정보량(채워진 컬럼 수) → ④ id 안정 정렬** 순으로 뽑아 `opportunity.representative_record_id`에 넣는다. 후보는 **`is_publishable` 멤버로 한정** — 민간 `pending`·`rejected`가 대표가 되거나 병합 결과를 오염시키지 않는다(§6-F). 민간이 낮은 건 ②뿐이므로, **마감된 공공 건과 진행 중인 승인 민간 건이 같은 그룹이면 민간이 대표가 된다**(①이 먼저다).
   - **후보가 비면 전체 멤버에서 `id` 최소를 둔다.** 멤버가 전부 `pending`·`rejected`인 그룹(마지막 승인 멤버를 재검수 지정한 직후가 그렇다)에서는 `is_publishable` 후보가 0개인데 `representative_record_id`는 `NOT NULL`이라 비울 수 없다 — 후보 제한만 읽고 구현하면 **재검수 트랜잭션이 거기서 실패한다.** 이때 대표가 무엇이든 `is_visible = false`라 노출되지 않으므로(불변식 1·2) 안정 정렬로 하나 고르면 된다.
...
4. **수동 태깅(민간 잔여분 — FR-011):** 민간 소스는 구조화 필드가 없고 본문 전문도 수집하지 않아 1·2·3 어느 단계도 못 채운다 → 검수자가 `opportunity.manual_*`에 직접 넣고, 병합이 이를 1~3의 결과와 **합집합**한다(§6-F 규칙 9). 사람의 판단이라 **`source_record`에는 쓰지 않는다** — 계층 경계(§2) 위반이고, 매 배치 UPSERT가 사람이 넣은 값을 덮어쓴다.
...
4. **재수집은 내용만 갱신한다 — `review_status`는 바뀌지 않는다**:
   ...
   **재검수 지정**(`approved` → `pending`, 아래 제목 경고 대응). **배치는 어느 쪽도 하지 않는다** —
   ...
   ON CONFLICT (source_code, external_id) DO UPDATE SET
       <내용 필드 전부> = EXCLUDED.<...>,     -- review_status는 SET 목록에 없다
       updated_at = now();
   ```

   **왜 승인을 되돌리지 않나.** 크롤러가 새로 읽어온 값이 어제 값보다 **정확하다.** 마감일이
   3월 10일에서 3월 25일로 바뀌었으면 새 값이 맞고, 그걸 이유로 공고를 숨기면 서비스가 더
   나빠진다. 검수가 판단한 건 **"이 공고를 우리 서비스에 실을 만한가"**이고, 그 판단은 마감일이나
   금액이 바뀐다고 달라지지 않는다.

   > 초안은 승인을 "그 시점 내용에 대한 승인"으로 보고 핵심 필드가 바뀌면 `pending`으로 되돌렸다
   > (강등). 되돌릴 필드 목록·되돌린 뒤 대표 재선정·수동 태그 처리까지 규칙이 딸려 오는데, 정작
   > 막는 사고는 아래 하나뿐이었다. 상태 기계를 없애고 리포트 경고로 옮긴다.

   **제목이 통째로 바뀌면 수집 리포트에 경고를 남긴다(강등하지 않는다).** 게시판이 글 번호를
   ...
   **자격 문구가 바뀌면 수동 태그를 비운다.** 민간은 구조화 필드가 없어 검수자가
   `eligibility_detail`을 읽고 페르소나를 매긴다(규칙 9). 그 문구가 바뀌면 검수자가 읽은 문장이
   달라진 것이므로 §2-C **불변식 6**이 적용된다 — `manual_*`를 비우고 재병합해(불변식 5) 태깅
   큐로 돌린다(규칙 10). 상태는 `approved` 그대로고 공고도 계속 노출된다. 공공 실체는 수동
   태그가 애초에 없으므로 이 규칙에 걸리는 게 없다.

5. **사람의 입력은 그가 본 상태에 대해서만 유효하다 (낙관적 동시성 — 판정과 태깅 전부)**:
   검수 CLI는 내용을 읽을 때 그 행의 `updated_at`을 함께 들고, 판정을 이렇게 건다.
   ...
   UPDATE source_record
    ...
    WHERE id = :id
      ...
      AND review_status = :expected_status;  -- 내가 본 그 상태 그대로인가
   ```
   ...
   **두 조건이 각각 다른 경합을 막는다**: `updated_at` 비교는 *수집 배치*와의, `review_status =
   :expected_status` 비교는 *다른 검수자*와의 경합을 막는다. `SET`에서 `updated_at`을 올리는 게 필수다 —
   ...
   **왜**: 검수자가 화면을 읽은 뒤 판정을 입력하기 전에 일일 배치가 같은 행을 UPSERT하면(그 행은
   내용만 갱신된다) 사람이 본 값과 확정되는 값이 달라진다. 승인 쪽은
   미검증 내용이 공개되는 문제이고, **반려 쪽은 더 나쁘다** — `rejected`는 이후 재수집에도
   불변이라 보지도 않은 새 내용이 영구 반려로 굳고 검수 큐에 다시 나타나지 않아 유실된다.
6. **검수 판정은 그 그룹의 재병합과 원자적이다**(§2-C 불변식 5의 사례). 승인 CLI는
   **한 트랜잭션 안에서** ① `review_status='approved'` → ② 해당 건의 dedup 판정(§6-D) →
   ③ 그룹 `opportunity` 재병합(대표·`is_visible` 확정) → ④ 수동 태깅(규칙 9)을 `manual_*`에
   기록하고 그 그룹의 `target_*`만 다시 합집합, 을 함께 커밋한다. ④가 ③ 뒤인 이유는 단독 민간
   공고의 `opportunity` 행이 ③에서 처음 생기기 때문이다.
   **재검수 지정도 같은 모양이다** — ①이 `approved` → `pending`이고 ②가 없을 뿐 **③은 반드시
   있다.** 없으면 `source_record` 상태만 바뀌고 단독 공고는 계속 노출되며, 그룹 공고는 미검수
   멤버의 대표값·금액을 그대로 내보낸다. ④는 태깅이 아니라 **태그 무효화**로 바뀐다 — **조건은 §2-C 불변식 6이 정한다**(여기에
   다시 적지 않는다. 두 곳에 적었다가 한쪽만 고치는 일이 실제로 반복됐다). 이걸 빼면 재병합이
   미검수 멤버의 값을 걸러도 **사람이 매긴 페르소나만 남아** 공공 멤버로 노출 중인 공고에 실린다. 커밋 후 웹 캐시 무효화까지가 한 동작인 것도 승인·반려와
   같다(PRD FR-011 9항).
...
9. **페르소나·금액**: 민간은 구조화 필드 없음 → 크롤러는 `source_record.target_*`를 NULL로 적재
   ...
   - **`source_record`에 안 쓰는 이유**: 그 계층은 매 배치 통째로 덮어써도 안전해야 하는데,
     수동 태그가 있으면 UPSERT가 사람의 판단을 지운다.
...
10. **검수 CLI**: `apps/ingest`의 poetry 스크립트(예: `python -m ingest.review`) — **목록이 두 개다.**
    ...
    - **태깅 큐** = **`approved` 민간 멤버를 가진 실체 중 `opportunity.manual_tagged_at IS NULL`**
      → 페르소나 태깅만. **검수 큐와 별개 목록이라는 게 핵심이다**: 자격 문구가 바뀌어 태그를
      비운 공고(규칙 4)는 멤버가 `approved` 그대로라 검수 큐에 영영 안 잡힌다. 큐가 하나뿐이면
      그 공고는 페르소나 탭에서 **조용히 계속 누락**된다(AC-041).
      - **조건이 "노출 중인가"가 아니라 "매길 근거가 있는가"인 이유.** `is_visible`을 기준으로
        삼으면, 공공 멤버 때문에 노출 중인 실체가 민간 멤버를 재검수 지정한 직후에도 큐에 남는다.
        작업자가 큐를 정상 처리하는 것만으로 **아직 `pending`인 멤버를 근거로 태그를 다시 기록**해,
        불변식 6이 방금 비운 미검수 페르소나가 그대로 되살아난다. 큐의 술어를 불변식 6과 **같은
        것**(`approved` 민간 멤버의 존재)으로 맞춰야 "비웠다가 곧바로 다시 채우는" 왕복이 없다.
      - `is_visible` 조건은 따로 걸지 않는다 — `approved` 민간 멤버가 있으면 `is_publishable`
        멤버가 있는 것이므로 불변식 1이 `is_visible = true`를 이미 보장한다.
    - 태깅 큐의 건을 `pending`으로 되돌려 검수 큐에 합치지 않는 이유: 내용 승인은 여전히
      유효한데 되돌리면 멀쩡한 공고가 재검수까지 며칠 사라진다. 페르소나 미상은 노출을 막을
