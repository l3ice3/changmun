...
5. `source_record.opportunity_id` 역참조 채우기 → `bookmark`에 `merged_from_id` 추가(이관 시점엔 전부 NULL — 이관은 그룹을 바꾸지 않는다) + `bookmark.opportunity_id` FK를 신 테이블로
...
4-B. **실체 배정 (survivor 규칙 — v2 신설).** 4의 파티션은 매 배치 다시 계산된다: `_seed_existing_groups`가 기존 그룹을 **재검증해 stale 그룹을 해제**하고, 블로킹 안에서 새 쌍이 붙는다(`dedup/engine.py`). v1은 `dedup_group_id`만 갈아 끼우면 끝이었지만 **v2는 그룹이 곧 행이라 id·북마크·수동 태그의 승계를 정해야 한다.** materialize의 UPSERT 키는 그룹이 아니라 여기서 정해지는 `opportunity.id`다.
   ...
   - **분할이 과거의 병합을 되돌리는 경우 — 새 id를 발급하지 않는다.** 갈라져 나온 멤버 집합이 **예전에 흡수된 실체 그대로면**(`opportunity_alias.old_id`가 그 멤버들의 직전 `opportunity_id`였다) **그 `old_id`를 되살려** 행을 만들고 alias 행을 지운다. 그러지 않으면 오합치를 해제한 뒤에도 **옛 상세 URL이 계속 틀린 실체로 리다이렉트**된다 — dedup의 "비파괴(그룹만 해제 가능)" 원칙이 실체층에서 깨진다.
