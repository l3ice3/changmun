...
6. **수동 태그의 근거를 더는 보장할 수 없으면 `manual_*`를 비우고 태깅 큐로 보낸다.**
   ...
   - **그 실체에 `approved` 민간 멤버가 하나도 없게 됐다**(재검수 지정·반려). 검수된 근거가
     사라졌는데 태그만 남으면, 공공 멤버로 계속 노출되는 공고의 페르소나 필터·응답에
     **미검수 판단이 그대로 실린다** — 불변식 4가 `manual_*`를 노출 자격과 무관하게 합치기
     때문이고, 단계 6의 donor 제한(`is_publishable`)으로는 안 걸린다. 승인 민간 멤버가 둘 중
     하나만 빠졌으면 남은 쪽이 근거이므로 유지한다.
