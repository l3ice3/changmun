...
4. **재수집은 내용만 갱신한다 — `review_status`는 바뀌지 않는다**:
   ...
   **자격 문구가 바뀌면 수동 태그를 비운다.** 민간은 구조화 필드가 없어 검수자가
   ...
   사라지므로 **그 실체의 `manual_*`·`manual_tagged_at`을 비우고 그 그룹의 `target_*`를 다시
   계산해**(불변식 5) 태깅 큐로 돌린다(규칙 10).
   상태는 `approved` 그대로고 공고도 계속 노출되며, 페르소나 탭에서만 "조건 미상"으로 빠진다 —
   태그가 낡았다고 멀쩡한 공고를 숨길 이유는 없다(절대규칙 8). 공공 실체는 수동 태그가 애초에
   없으므로 이 규칙에 걸리는 게 없다.
...
6. **검수 판정은 그 그룹의 재병합과 원자적이다**(§2-C 불변식 5의 사례). 승인 CLI는
   ...
   **재검수 지정도 같은 모양이다** — ①이 `approved` → `pending`이고 ②·④가 없을 뿐 **③은 반드시
   ...
   멤버의 대표값·금액을 그대로 내보낸다. 커밋 후 웹 캐시 무효화까지가 한 동작인 것도 승인·반려와
   같다(PRD FR-011 9항).
