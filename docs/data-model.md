...
5. **검수 판정은 사람이 본 스냅샷에 대해서만 유효하다 (낙관적 동시성 — 승인·반려·재검수 지정 전부)**:
...
6. **검수 판정은 그 그룹의 재병합과 원자적이다**(§2-C 불변식 5의 사례). 승인 CLI는
   ...
   **재검수 지정도 같은 모양이다** — ①이 `approved` → `pending`이고 ②가 없을 뿐 **③은 반드시
   ...
   멤버의 대표값·금액을 그대로 내보낸다. ④는 태깅이 아니라 **태그 무효화**로 바뀐다 — 그 실체에
   `approved` 민간 멤버가 하나도 남지 않으면 `manual_*`를 비운다(불변식 6). 이걸 빼면 재병합이
