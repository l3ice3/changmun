...
CREATE INDEX idx_merge_log_survivor ON opportunity_merge_log (survivor_id);
```

> **왜 컬럼이 아니라 병합 단위 이력인가.** 처음에는 `source_record`·`bookmark`에 "옮겨지기 직전
> id" 컬럼 하나씩을 두려 했다. 한 단계 병합에는 충분하지만 **중첩되면 무너진다** — `B`가 `A`에
> 흡수되고 그 `A`가 다시 `C`에 흡수되면, "이미 값이 있으면 유지" 규칙 때문에 A의 원래 멤버만
> `A`를 가리키고 B의 멤버는 계속 `B`를 가리킨다. 두 번째 병합만 되돌려 `{A,B}`가 C에서 갈라져도
...
4. 신 `opportunity` + `opportunity_merge_log` 생성 후 그룹별 1행 INSERT — **id는 현재
...
4-B. **실체 배정 (survivor 규칙 — v2 신설).** 4의 파티션은 매 배치 다시 계산된다: `_seed_existing_groups`가 기존 그룹을 **재검증해 stale 그룹을 해제**하고, 블로킹 안에서 새 쌍이 붙는다(`dedup/engine.py`). v1은 `dedup_group_id`만 갈아 끼우면 끝이었지만 **v2는 그룹이 곧 행이라 id·북마크·수동 태그의 승계를 정해야 한다.** materialize의 UPSERT 키는 그룹이 아니라 여기서 정해지는 `opportunity.id`다.
   - **분할** (한 실체의 멤버가 두 그룹으로 갈림): 기존 `representative_record_id`를 **포함한 쪽이 기존 id를 승계**하고(포함한 쪽이 없으면 멤버 `first_seen_at`이 이른 쪽), 나머지는 새 행. `manual_*`는 **양쪽 모두 비운다**(`manual_tagged_at`까지 NULL) — 사람이 어느 쪽을 보고 판단했는지 알 수 없기 때문이다. 이 근거는 **대칭**이다: 갈라져 나온 쪽만 비우고 승계 쪽에 태그를 남기면, 그 태그가 **흡수됐던 다른 실체에서 온 것일 때**(병합이 `manual_*`를 합집합했으므로) 승계 쪽이 틀린 페르소나 탭에 계속 노출되고, `manual_tagged_at`이 남아 있어 태깅 큐에도 안 잡힌다. 여전히 유효했을 태그까지 버리는 손해는 있지만, **틀린 페르소나로 노출하는 것보다 "조건 미상"이 낫다**(가드레일 2·절대규칙 8). 재태깅 부담은 민간 공고량이 소스당 연 1~5건이라 작다. **두 공고 모두 `pending`으로 강등하지는 않는다**: 내용에 대한 승인은 여전히 유효하고(원문이 바뀐 게 아니다) 강등하면 멀쩡한 공고가 며칠 사라진다. 페르소나만 "조건 미상"으로 노출되며, **태깅 큐**(§6-F 규칙 10 — `manual_tagged_at IS NULL AND is_visible`)가 이 건을 잡는다. 검수 큐(`review_status='pending'`)와는 다른 목록이다.
   - **분할이 과거의 병합을 되돌리는 경우 — 새 id를 발급하지 않는다.** 갈라져 나온 멤버 집합이 `opportunity_merge_log`의 **어느 행의 `record_ids`와 정확히 일치**하면, 그 행의 `absorbed_id`를 되살려 실체를 만들고 · `bookmark_ids`의 찜을 그 id로 돌려놓고 · 그 로그 행을 지운다. 그러지 않으면 오합치를 해제한 뒤에도 **옛 상세 URL이 계속 틀린 실체로 리다이렉트되고 찜도 엉뚱한 공고에 남는다** — dedup의 "비파괴(그룹만 해제 가능)" 원칙이 실체층에서만 깨진다.
     - **후보가 여럿이면 `merged_at`이 가장 늦은 행**을 고른다. 병합은 스택처럼 쌓이므로 되돌리는 순서도 역순이다(`B→A` 후 `A+B→C`인 상태에서 `{A,B}`가 갈라지면 되살릴 것은 `A`이지 `B`가 아니다).
     - **조건은 정확히 "그대로"다** — 일부만 갈라졌거나 병합 이후 새로 붙은 멤버가 섞였으면 **새 id를 발급한다**: 되살린 id가 예전과 다른 공고를 가리키게 되는 편이, 그냥 새 URL이 생기는 것보다 나쁘다(리다이렉트가 거짓말을 한다).
     - **한계 — 중복으로 삭제됐던 찜은 못 되살린다**: 양쪽을 다 찜했던 사용자의 행은 병합 때 하나로 합쳐졌고(아래 ②), 그 행은 한 실체만 가리킬 수 있다. 그 사용자에게는 생존 실체 쪽 찜이 남으므로 전부 잃지는 않는다. `bookmark_ids`에는 **실제로 옮겨진 행만** 담기므로 복구도 그만큼이다.
   - **병합** (새 그룹에 기존 실체가 2개 이상): **survivor = ① 멤버 `first_seen_at` 최소 → ② `id` 최소.** 오래된 id일수록 색인·북마크가 더 붙어 있다. 나머지 행은:
     ① `source_record.opportunity_id`를 survivor로 재배정 → ② `bookmark.opportunity_id`를 survivor로 리매핑(같은 사용자가 양쪽을 찜했으면 `UNIQUE(user_id, opportunity_id)` 충돌 → **기존 행을 남기고 중복만 삭제**, 찜이 사라지지 않는다) → ③ `manual_*`는 **합집합**(한쪽만 있으면 그 값, `manual_tagged_at`은 더 늦은 쪽 — 최근 판단을 반영) → ④ `opportunity_merge_log`에 **이 병합 1행**을 남긴다: `absorbed_id` · `survivor_id` · `record_ids`(①에서 옮긴 멤버) · `bookmark_ids`(②에서 **실제로 옮겨진** 찜) → ⑤ 흡수된 행 DELETE. **②를 ⑤보다 반드시 먼저** — `bookmark.opportunity_id`는 `ON DELETE CASCADE`라(`V20260704_1500__create_bookmark.sql`), 리매핑 전에 행을 지우면 **그 실체를 찜한 사용자의 찜이 에러 없이 조용히 사라진다.**
   - **왜 로그가 필요한가**: 병합은 불변식 3의 유일한 예외라 id가 실제로 사라진다. 그대로 두면 **색인된 상세 URL이 404**가 되고 외부 링크·검색 유입이 끊긴다. 상세 라우트는 `opportunity`에 없으면 로그의 `absorbed_id`를 찾아 **생존 실체로 영구 리다이렉트**한다. 병합은 드물지만(오합치 > 놓침 원칙으로 임계 0.85) 한 번 끊긴 SEO URL은 되돌릴 수 없다.
...
10. **검수 CLI**: `apps/ingest`의 poetry 스크립트(예: `python -m ingest.review`) — **목록이 두 개다.**
    ...
    - **태깅 큐** = `opportunity.manual_tagged_at IS NULL AND is_visible`이면서 민간 멤버를 가진 실체
      → 페르소나 태깅만. **승인 여부와 무관한 별도 목록이라는 게 핵심이다**: 그룹 분할로 태그를
      잃은 공고(§6-D 단계 4-B)는 멤버가 `approved` 그대로라 검수 큐에 영영 안 잡힌다. 큐가
      하나뿐이면 그 공고는 페르소나 탭에서 **조용히 계속 누락**된다(AC-045).
...
>
> 병합으로 찜이 옮겨간 이력은 `bookmark`가 아니라 `opportunity_merge_log.bookmark_ids`에 남는다
> (§6-D 단계 4-B) — 그래야 중첩 병합에서 **단계별로** 되돌릴 수 있다. `bookmark` 스키마는 v1 그대로다.
