...
CREATE TABLE source_record (
    ...
    merged_from_opportunity_id BIGINT,           -- 그룹 병합으로 옮겨지기 직전의 opportunity_id
                                                 -- (§6-D 단계 4-B 분할 복구용 provenance. 서빙 미사용)
...
CREATE INDEX idx_source_record_merged_from ON source_record (merged_from_opportunity_id)
       WHERE merged_from_opportunity_id IS NOT NULL;   -- 분할 복구 시 옛 멤버 집합 조회
```
...
3. `id`는 **대표가 바뀌어도 불변**이다. v1은 canonical이 교체되면 `/opportunities/{id}`의 id가
   ...
   **유일한 예외는 그룹 병합**(두 실체가 같은 공고로 판정되는 경우)이며, 그때도 사라지는 id는
   `opportunity_alias`에 남아 상세가 survivor로 리다이렉트된다(§6-D 단계 4-B).
...
```sql
-- 그룹 병합으로 흡수돼 사라진 실체 id → survivor. 상세 라우트의 영구 리다이렉트 근거(§6-D 단계 4-B).
CREATE TABLE opportunity_alias (
    old_id      BIGINT      PRIMARY KEY,          -- 사라진 opportunity.id (FK 아님 — 대상 행이 없다)
    new_id      BIGINT      NOT NULL REFERENCES opportunity (id) ON DELETE CASCADE,
    merged_at   TIMESTAMPTZ NOT NULL DEFAULT now()
);
```

> alias가 또 병합되면 `new_id`를 **survivor로 갱신**한다(체인을 만들지 않는다 — 조회가 재귀가 되고
> 리다이렉트가 여러 홉이 된다). `new_id` 행이 지워질 일은 없지만, `ON DELETE CASCADE`로 매달아
> 고아 alias가 남지 않게 한다.
...
4. 신 `opportunity` + `opportunity_alias` 생성 후 그룹별 1행 INSERT — **id는 현재
...
5. `source_record.opportunity_id` 역참조 채우기 → `bookmark`에 `merged_from_id`, `source_record`에 `merged_from_opportunity_id` 추가(**이관 시점엔 둘 다 전부 NULL** — 이관은 그룹을 바꾸지 않는다) + `bookmark.opportunity_id` FK를 신 테이블로
...
4-B. **실체 배정 (survivor 규칙 — v2 신설).** 4의 파티션은 매 배치 다시 계산된다: `_seed_existing_groups`가 기존 그룹을 **재검증해 stale 그룹을 해제**하고, 블로킹 안에서 새 쌍이 붙는다(`dedup/engine.py`). v1은 `dedup_group_id`만 갈아 끼우면 끝이었지만 **v2는 그룹이 곧 행이라 id·북마크·수동 태그의 승계를 정해야 한다.** materialize의 UPSERT 키는 그룹이 아니라 여기서 정해지는 `opportunity.id`다.
   - **분할** (한 실체의 멤버가 두 그룹으로 갈림): 기존 `representative_record_id`를 **포함한 쪽이 기존 id를 승계**하고(포함한 쪽이 없으면 멤버 `first_seen_at`이 이른 쪽), 나머지는 새 행. `manual_*`는 **승계 쪽만** 가져가고 갈라져 나온 쪽은 NULL로 시작한다 — 사람이 어느 쪽을 보고 판단했는지 알 수 없으므로 미태깅으로 두어 검수 큐에 다시 잡히는 게 맞다.
   - **분할이 과거의 병합을 되돌리는 경우 — 새 id를 발급하지 않는다.** 갈라져 나온 멤버 집합이 **예전에 흡수된 실체 그대로면 그 `old_id`를 되살려** 행을 만들고, 해당 멤버들의 `merged_from_opportunity_id`를 비우고 alias 행을 지운다. 그러지 않으면 오합치를 해제한 뒤에도 **옛 상세 URL이 계속 틀린 실체로 리다이렉트**된다 — dedup의 "비파괴(그룹만 해제 가능)" 원칙이 실체층에서만 깨진다.
     - **판정에는 멤버 provenance가 필요하다.** 병합이 `source_record.opportunity_id`를 survivor로 덮어쓰기 때문에, `opportunity_alias`의 `old_id → survivor`만으로는 **어떤 멤버가 그 `old_id`에 속했었는지 복원할 수 없다.** B·C가 A에 흡수된 뒤 B만 갈라져도 alias 두 줄로는 B의 멤버 집합을 고를 수 없다. 그래서 병합 시 `source_record.merged_from_opportunity_id`에 **덮어쓰기 직전의 `opportunity_id`를 남긴다**(§2-B).
     - **조건은 정확히 "그대로"다** — 갈라져 나온 집합이 `merged_from_opportunity_id = X`인 멤버 **전체와 정확히 일치**할 때만 X를 되살린다. 일부만 갈라졌거나 병합 이후 새로 붙은 멤버가 섞였으면 **새 id를 발급한다**: 되살린 id가 예전과 다른 공고를 가리키게 되는 편이, 그냥 새 URL이 생기는 것보다 나쁘다(리다이렉트가 거짓말을 한다).
   - **북마크도 되돌린다.** 그러려면 병합 시점에 출처가 남아 있어야 하므로 `bookmark`에 `merged_from_id`(병합 전 이 찜이 가리키던 실체 id, nullable)를 둔다. 분할로 `old_id`가 되살아나면 `merged_from_id = old_id`인 찜을 그 id로 되돌리고 컬럼을 NULL로 비운다. **한계 — 중복으로 삭제됐던 찜은 못 되살린다**: 양쪽을 다 찜했던 사용자의 행은 병합 때 하나로 합쳐졌고, 남은 행 하나가 두 출처를 표현할 수 없다. 그 사용자에게는 survivor 쪽 찜이 남으므로 전부 잃지는 않는다. 이 비대칭을 감수하는 이유는, 되살리려면 `bookmark`에 이력 테이블을 붙여야 하는데 **병합→분할은 임계 0.85·전수 검수 오합치 0을 통과한 뒤 뒤늦게 갈라지는 드문 사건**이라 비용이 맞지 않기 때문이다.
   - **병합** (새 그룹에 기존 실체가 2개 이상): **survivor = ① 멤버 `first_seen_at` 최소 → ② `id` 최소.** 오래된 id일수록 색인·북마크가 더 붙어 있다. 나머지 행은:
     ① `source_record.opportunity_id`를 survivor로 재배정하며 **`merged_from_opportunity_id`에 덮어쓰기 직전 값을 기록**(이미 값이 있으면 유지 — 최초 출처를 보존해 체인을 만들지 않는다. survivor 자신의 멤버는 옮겨지지 않으므로 NULL로 남는다) → ② `bookmark.opportunity_id`를 survivor로 리매핑하며 **`merged_from_id`에 옛 id 기록**(이미 값이 있으면 유지 — 최초 출처를 보존해 체인을 만들지 않는다. 같은 사용자가 양쪽을 찜했으면 `UNIQUE(user_id, opportunity_id)` 충돌 → **기존 행을 남기고 중복만 삭제**, 찜이 사라지지 않는다) → ③ `manual_*`는 **합집합**(한쪽만 있으면 그 값, `manual_tagged_at`은 더 늦은 쪽 — 최근 판단을 반영) → ④ `opportunity_alias(old_id → survivor)` 기록 후 행 DELETE. **②를 ④보다 반드시 먼저** — `bookmark.opportunity_id`는 `ON DELETE CASCADE`라(`V20260704_1500__create_bookmark.sql`), 리매핑 전에 행을 지우면 **그 실체를 찜한 사용자의 찜이 에러 없이 조용히 사라진다.**
   - **왜 alias가 필요한가**: 병합은 불변식 3의 유일한 예외라 id가 실제로 사라진다. 그대로 두면 **색인된 상세 URL이 404**가 되고 외부 링크·검색 유입이 끊긴다. 상세 라우트는 `opportunity`에 없으면 alias를 보고 **survivor로 영구 리다이렉트**한다. 병합은 드물지만(오합치 > 놓침 원칙으로 임계 0.85) 한 번 끊긴 SEO URL은 되돌릴 수 없다.
   - 4-B → 5 → 6은 **한 트랜잭션**이다. 중간에 끊기면 `opportunity_id`가 재배정됐는데 실체 행이 없는 멤버가 남는다.
...
CREATE TABLE bookmark (
    ...
    merged_from_id BIGINT,                       -- v2: 병합 전 이 찜이 가리키던 실체 id(§6-D 단계 4-B)
    created_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
...
>
> `merged_from_id`는 **그룹 병합으로 찜이 옮겨간 사실만 기록**한다(§6-D 단계 4-B). 나중에 그
> 병합이 분할로 되돌려지면 이 값으로 원래 실체에 찜을 돌려놓는다 — 없으면 오합치를 해제해도
> 사용자의 찜은 엉뚱한 공고에 남는다. **서빙·API에는 노출하지 않는다**(내부 이력 컬럼 —
> api-spec의 찜 응답 계약은 그대로다).
