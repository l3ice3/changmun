...
3. **리스트/검색 쿼리에 `is_canonical = true` 고정.** `ids=` 조회만 예외. **`(review_status IS NULL OR review_status = 'approved')`는 예외 없이 전 경로** — 리스트·검색·상세·`ids=`·**stats 집계 3종**까지(FR-010, data-model §6-F 규칙 1. stats는 `is_canonical`만 세던 곳이라 빠뜨리기 쉽다).
