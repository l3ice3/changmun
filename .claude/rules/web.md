...
10. **ISR 24h 전제 = "상태 변경은 일 1회 배치뿐"**. FR-011 민간 검수는 이 전제를 깬다(사람이 배치 주기 밖에서 승인·반려) → 검수 판정 시 해당 공고의 **캐시를 즉시 무효화**해야 한다(PRD FR-011 9항). 무효화 없이는 반려한 공고가 **최대 하루 더 노출**돼 가드레일 2가 웹에서 무력화된다. 구현 시 on-demand revalidation 라우트가 필요(현재 레포에 없음).
